...
function loadStatFile(fileName)
	...
	local function processLine(line)
		local include = line:match('include "Metadata/Balance/StatDescriptions/(.+)"$')
		if include then
			local text = convertUTF16to8(getFile("Metadata/Balance/StatDescriptions/"..include))
	...
	local text = convertUTF16to8(getFile("Metadata/Balance/StatDescriptions/"..fileName))
	for line in text:gmatch("[^\r\n]+") do
		processLine(line)
	end
